...
/*   Updated: 2022/12/15 20:35:06 by ebensalt         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*lexer_get_value_norm1(t_lexer *lexer, int type, char *value)
{
	value = lexer_get_value_norm2(lexer, type, value);
	if (type == EXIT_C)
	{
		lexer_advance(lexer);
...
char	*lexer_get_value(t_lexer *lexer, int type)
{
	...
	value = lexer_get_value_norm1(lexer, type, value);
	return (value);
}
