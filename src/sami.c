...
/*   Updated: 2022/12/28 13:10:43 by ebensalt         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	change_pwd(t_list *list, char *s)
{
	...
	if (!ft_strcmp(list->first, s))
		list->second = ft_strdup(getcwd(NULL, 0));
}
...
void	list_checker(t_cmd *cmd, t_list *list)
{
	...
	while (cmd->cmd_line[++j])
	{
		...
		else
		{
			...
			if (!str || (cmd->cmd_line[j][i + 1] != 0 && ft_strcmp(str, &cmd->cmd_line[j][i + 1])))
			{
				delete_env(&list, ptr);
				add_list_nood(&list, cmd->cmd_line[j]);
			}
